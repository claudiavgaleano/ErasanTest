...
export default function SpecialProjects() {
  ...
  const { categories } = useCategories('specialProjects')
        ...
        <Container maxWidth="lg">
          <Box sx={{ textAlign: 'center', maxWidth: 800, mx: 'auto', position: 'relative', zIndex: 1 }}>
            ...
            <Typography variant="h1" sx={{ fontSize: { xs: '2.5rem', md: '3.5rem' }, mb: 3, fontWeight: 700 }}>
              ...
              >
                {t('specialProjects.heroTitle2')}
              </Box>
            </Typography>
            <Typography variant="body1" color="text.secondary" sx={{ fontSize: '1.1rem', lineHeight: 1.8 }}>
              {t('specialProjects.heroDescription')}
            </Typography>
          </Box>
        </Container>
      </Box>
      ...
      {categories.length > 0 && (
        <Container maxWidth="lg" sx={{ mb: 4 }}>
          <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap', justifyContent: 'center' }}>
            <Chip
              label={t('specialProjects.allCategories')}
        ...
        <Container maxWidth="lg">
          {error && (
            <Alert severity="error" sx={{ mb: 4 }}>
              {t('specialProjects.errorLoading')}
            </Alert>
          )}
                    ...
                    >
                      <Box sx={{ overflow: 'hidden', position: 'relative' }}>
                        ...
                        {product.acf?.featured && (
                          <Chip
                            label={t('specialProjects.featured')}
                            color="primary"
                            size="small"
                            sx={{
                              position: 'absolute',
      ...
      >
        <Container maxWidth="md" sx={{ textAlign: 'center' }}>
          <Typography variant="h2" sx={{ mb: 2, fontWeight: 600 }}>
            {t('specialProjects.ctaTitle')}
          </Typography>
          <Typography variant="body1" color="text.secondary" sx={{ mb: 4 }}>
            {t('specialProjects.ctaDescription')}
          </Typography>
          <Button
            variant="contained"
            size="large"
            ...
            endIcon={<ArrowForwardIcon />}
          >
            {t('specialProjects.requestQuote')}
          </Button>
        </Container>
      </Box>
    </Box>
